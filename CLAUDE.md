...
- **WhatsApp/Interakt: `WHATSAPP_API_KEY` live in `.env.local` as of Jul 8** (account live, number connected via Meta, FB Business Manager verified). Code side is fully built and waiting (`lib/whatsapp/index.ts` — `sendTemplate`, `verifyDeliveryWebhook`, all 4 trigger points already wired, currently a graceful no-op). **The real remaining blocker: the 4 templates have not been submitted to Meta at all yet** (confirmed Jul 8) — a valid API key sends nothing until WhatsApp Cloud API has an approved template to send, which is a 1–7 day review once submitted. To fully activate:
  1. **Submit the 4 drafts in `docs/whatsapp-templates.md` to Meta via Interakt's template manager** — this is the actual next action, and the clock hasn't started.
  2. Once approved, confirm the **template names on Meta exactly match** the code's `WhatsappTemplateKey`s — `order_placed`, `delivered`, `return_received`, `refund_processed` (case-sensitive; the API call sends `template.name` = these literal strings). If named differently during submission, either rename in Meta or update `WhatsappTemplateKey` to match.
  3. `WHATSAPP_WEBHOOK_SECRET` (optional but recommended) — if Interakt issues a signing secret for delivery/read-receipt webhooks. The webhook URL to give Interakt is `https://<deployed-domain>/api/webhooks/whatsapp` (route already built) — needs the real deployed URL, so it's a post-deploy step.
  4. `WHATSAPP_API_BASE_URL` — only needed if Interakt's base URL differs from the code's default (`https://api.interakt.ai/v1/public`); otherwise leave unset.
...
- [ ] WhatsApp BSP (Interakt) — **code complete**, `WHATSAPP_API_KEY` live in `.env.local` as of Jul 8. `sendTemplate()` (Interakt) + all 4 trigger points wired (order_placed, delivered, return_received, refund_processed), opt-in gated via `orders.whatsapp_opt_in`, every send logged to `message_log`. **Blocked on Meta template approval — templates have not been submitted yet** (confirmed Jul 8). Template drafts: `docs/whatsapp-templates.md` — submit to Meta via Interakt's template manager now; 1–7 day review.
...
- **Meta template approval — NOT YET SUBMITTED as of Jul 8, confirmed.** `WHATSAPP_API_KEY` is live and in `.env.local`, but that alone does nothing: WhatsApp Cloud API rejects sending any template that hasn't been submitted to and approved by Meta first — this is a hard platform requirement, independent of the Interakt key. The 1–7 day review clock hasn't started. Submit the 4 drafts in `docs/whatsapp-templates.md` to Meta (via Interakt's template manager) ASAP — this is now the most time-sensitive open item since it's the only remaining thing on a multi-day external clock.
- ~~Interakt API-key access blocked~~ **Cleared Jul 8** — credentials live, key is in `.env.local`. Templates are the remaining gate (see above).
