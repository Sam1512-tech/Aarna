...
- **⚠️ OPEN DECISION — return window is inconsistent three ways:** backend enforces **14 days** (`lib/actions/account.ts` + both account pages), return-policy page + FAQ say **3 days**, WhatsApp `delivered` template says **7 days** (`app/api/webhooks/delhivery/route.ts`). Arpitha must pick one number; then align all three surfaces. Do this before launch — the policy page is the legally binding one.
...
- **Return window inconsistency (14 vs 3 vs 7 days)** — see ⚠️ OPEN DECISION in the audit section above.
