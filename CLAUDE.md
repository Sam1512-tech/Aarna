...
**THE APP IS DEPLOYED AND FULLY LIVE: https://shopaarna.in** (Vercel, Sam's account `sam1512-tech`, project `aarna`, **Pro plan** — upgraded from Hobby; unlocks per-minute cron granularity, see "Delivery-status auto-sync" below). Auto-deploys on every merge to `main`. **Full-scope audit completed Jul 4** (27 checks); **admin CRUD + reviews closed out Jul 7**; **coupon bug, Razorpay key rotation, tag printing shipped Jul 8**; **deployed + 12-PR Vismaya batch merged + all 4 WhatsApp templates approved Jul 8–10**; **second full code audit Jul 11** (see below); **Vismaya's 5-branch polish batch + cart-badge follow-ups merged Jul 15**; **cart bug-fix batch + PDP fixes Jul 16**; **round-2 full audit (77 findings) + all 3 critical + all 17 high-severity fixes merged Jul 22–23**; **round-3 batch — 22 PRs covering the medium-severity backlog, the pre-launch coming-soon gate, and a critical Google OAuth bug — merged Jul 23–24** (see below). State of the world:

### Return/refund investigation + delivery-status auto-sync (Aug 2, PR #295)
- **First real customer return processed (AARNA-001067)** — while checking it, found the actual refund (₹2,598, incl. the ₹99 shipping fee) had been issued manually via the Razorpay dashboard rather than through `/studio/returns`' QC flow, leaving the return row stuck at `"received"` with no `qc_outcome`/`resolved_at`/refund link, and inventory never restocked (only `markReturnQc`'s pass path restocks — never ran). Confirmed against Razorpay's own API (`rfnd_TK2wkan7GATNY9`, empty `notes` — the app's own refunds always tag `{returnId, orderNumber}`, so this definitively didn't go through the app). Return row backfilled by hand (status/qc_outcome/refund ID/amount/resolved_at) after confirming with Sam; inventory restock left to Sam. **Process note, not a bug**: refunds must go through QC in `/studio/returns`, never the Razorpay dashboard directly — that's what applies the correct refund amount (item only, never shipping) and keeps the return record in sync.
- **Tax invoices are never altered/deleted on refund, by design** — `buildInvoiceData` always rebuilds from the order's original stored values, so a refunded order's invoice still (correctly) shows what was actually charged/invoiced at the time. The proper accounting treatment (a credit note referencing the original invoice) **isn't built** — known pre-existing gap, already flagged in a comment on `lib/actions/admin/reports.ts:115`. Flag refunds to the accountant manually for now; credit-note generation would be a real standalone feature if wanted later.
- **Diagnosed why delivered orders weren't updating on the site**: confirmed directly against Delhivery's own tracking API that a real delivery (AARNA-001059, delivered Jul 31) never updated `fulfillment_status` — the Delhivery status webhook has stopped reaching production. Root cause is almost certainly the webhook URL never being (re-)registered in Delhivery's own merchant panel — a one-time dashboard step, not a code fix; the webhook code + `DELHIVERY_WEBHOOK_TOKEN` are already correct in Vercel prod. **Still needs Sam to register `https://shopaarna.in/api/webhooks/delhivery?token=<DELHIVERY_WEBHOOK_TOKEN>` in the Delhivery panel** (Settings → API → Webhooks, or similar) for genuinely instant updates — nothing in code can do this step.
- **Separately found: `CRON_SECRET` was missing from Vercel production entirely.** The existing daily cron (`app/api/cron/cleanup-orders`) has been returning `500` on every invocation since it shipped Jul 22 — unpaid-order cleanup, checkout-hold release, rate-limit cleanup, and stale-shipment alerting have never actually run in production. Fixed by adding the secret to Vercel prod + `.env.local`.
- **Built an automated fallback that doesn't depend on the Delhivery dashboard fix**: `syncInFlightShipmentStatuses` (`lib/db/queries/orders.ts`) checks Delhivery's real tracking API for every order still `shipped`/`out_for_delivery` and applies any status change through `applyDelhiveryStatus` — the same shared logic the real-time webhook now also uses, so the two paths can never drift apart. Fixed a real latent bug in the process: the webhook's "delivered" WhatsApp had no guard against re-firing on a repeat "delivered" observation (retries), unlike the sibling "returned" RTO-alert branch, which already correctly guarded on `fulfillmentStatus !== "returned"`.
- **This is its own cron (`/api/cron/sync-delivery-status`), deliberately separate from `cleanup-orders`** — originally bundled into the one existing cron (matching the project's established "ride the existing daily cron" convention), but split out once **the Vercel plan turned out to already be Pro, not Hobby** as this file previously said. Hobby caps *every* cron at once/day, no exceptions (confirmed directly against Vercel's current docs) — Pro allows down to once/minute. Bundling would have forced `alertStaleShipments` (which deliberately re-alerts daily, not on a "first time only" basis — see its own comment) to also run every few minutes, spamming admin inboxes. Scheduled at `*/5 * * * *` (every 5 minutes) in `vercel.json` — tight enough to feel close to real-time, without hammering Delhivery's API pointlessly at current order volume. Adjust the interval if that trade-off ever needs revisiting.
- **Production-data safety note for future sessions**: the site is live with real orders/money and no separate staging DB — a local dev server here still points at the same real database. Read-only investigation is fine; anything that mutates real rows (including running this new cron with a valid secret, or a real webhook push) needs explicit confirmation first, same as any other destructive/high-blast-radius action.
...
- **`#267`** — the modal-lock fix above caused a NEW, real regression: `document.body.style.overflow = "hidden"` removes the scrollbar, growing the viewport's effective width by exactly the scrollbar's width (measured live: 1265px → 1280px, an exact 15px jump) — so every `fixed inset-x-0` element (header, marquee) visibly shifted the instant any modal opened. Fixed with `scrollbar-gutter: stable` on `html` in `app/globals.css`, reserving that space permanently so the width never changes. Verified live on 3 structurally-different modal instances, including a disposable Supabase test-customer account to exercise the actual "Add address" modal.
- **`#269`** — three issues in the Delhivery fulfillment flow: (1) the customer "Track shipment" link (`/account/orders`) used Delhivery's old `/tracking/<awb>` path, which 301s to Delhivery's own literal 404 — fixed to `/track-v2/package/<awb>` (confirmed HTTP 200, real tracking page), and stopped rendering during the internal `"PENDING"` claim-sentinel window; (2) "package weight always shows 450g" does not reproduce — verified live against real multi-line dev-DB orders that `createDelhiveryShipment` already sums per-line `weightGrams` (450g fallback only when a variant has none set) × quantity correctly; the real gap is that 0 of 55 launch variants have ever had `weightGrams` set (no admin UI field exists for it — a legitimate, separate follow-up); (3) **found and fixed a real bug**: `attachAwbNumber` (the manual "paste AWB" recovery action, separate from `createDelhiveryShipment`'s already-race-safe shipment creation) had zero protection against overwriting an existing AWB — had already corrupted order `AARNA-001034`'s `awb_number` into a real waybill concatenated with itself (`"55173410000044"` → `"5517341000004455173410000044"`), most likely a barcode double-scan into the always-pre-filled input. Data corrected directly; action now rejects overwriting a real AWB unless `{ replace: true }` is explicitly passed, with the DB write itself conditional on the AWB still matching what was read (mirroring `createDelhiveryShipment`'s atomic-claim pattern) so concurrent attach/replace calls can't race each other — verified live via two simultaneous requests producing exactly one winner. Admin UI now shows the current AWB read-only with a separate, confirmation-gated "Replace shipment" action.
...
### Independent adversarial re-audit of the architecture review — 2 real races found + fixed (Jul 24–25, PR #250)
After a full architecture review (scored 79/100, verdict "Conditional GO"), a second pass was deliberately run with instructions to distrust the first report entirely and independently re-verify every claim — plus a genuinely blind fresh audit and a dedicated red-team pass trying to actively break the system. Most of the original findings held up (severities recalibrated on a couple, one supporting example was factually wrong, one "zero import leaks" strengths claim turned out to have two narrow documented exceptions). What actually mattered: the red-team pass found **two real, previously-undiscovered bugs in money-critical code** that the architecture review's methodology was never built to catch — an architecture review checks structure and layering; it doesn't construct exploits. Both were personally re-traced from source (not taken on the sub-agents' word) before fixing. **Fixed and live in prod as of PR #250:**

- **Checkout-hold-release oversell race.** The `payment.captured` webhook computed whether to re-decrement stock from an `order` snapshot read **before** `markOrderPaid`'s guard ran. `releaseExpiredCheckoutHolds()` flips a stale order to `"failed"` and restocks +1 but never resets `stockReserved` — so if a release landed between the webhook's read and its guard (a late payment capture arriving >20 min after order creation, which this project's own "Floral Corset Dress" incident proves isn't hypothetical), the webhook's stale snapshot still said "stock already reserved, nothing to do," and the +1 given back was never taken back. Result: a real, paid, shipped order with the same unit simultaneously available to sell again — no alert, no detection. **Fixed by moving the decision inside `markOrderPaid`'s own transaction**: it now `SELECT...FOR UPDATE`-locks the order row and derives `needsStockDecrement` from a fresh, locked read of current state, instead of trusting a pre-guard snapshot the caller read separately — the same pattern `initCheckout`/`applyStockMovement`/`markReturnQc` already use correctly.
- **Stale-order permanent-deletion race.** `deleteStaleUnpaidOrders()` sequentially checks each 7-day-stale candidate against Razorpay, then runs one final unconditional `DELETE` using ids collected across the *whole* loop. A genuinely late `payment.captured` webhook landing for an order already cleared earlier in the loop — while later candidates were still being checked — got deleted anyway, cascading to wipe its `order_items` (`onDelete: cascade`): a real, paid, invoiced, emailed order permanently vanishing with zero trace. **Fixed with a fresh re-select of `payment_status` immediately before acting, plus the delete's own `WHERE` clause re-checking status atomically at the instant it runs** — the actual guarantee that makes an irreversible delete safe. An order saved by a late capture is now excluded and reported via `alertAdminSuspiciousActivity` instead of silently destroyed.

Both fixes verified live against the real dev DB with a full synthetic fixture (category/product/variant/orders) reproducing each race end-to-end — for the first, the *old* buggy decision was computed explicitly for comparison (would have left stock oversold) against what the fix correctly computes; for the second, the atomic guard was proven to exclude a since-paid order while still correctly deleting a genuinely stale one. 11/11 assertions passed, all test fixtures cleaned up. Production auto-deploy succeeded cleanly.

### Storefront polish — PLP count wording, un-stuck filter bar, navbar balance (Jul 24, PR #253)
Three independent frontend fixes, each blast-radius-mapped first per the Fix Protocol:

- **PLP count label** — "N piece(s)" → "N product(s)" in `components/storefront/plp-view.tsx`'s toolbar. `PlpView` is the ONE shared component behind `/shop`, `/shop/[category]`, and `/collections/[slug]` (confirmed via every importer), so this single edit covers all three consistently. `/search` has its own, deliberately different "N results for…" label in a separate component (`search-view.tsx`) — checked and correctly left alone, since it's query-match wording, not category-browsing wording.
- **Sticky filter bar removed** — same toolbar div; dropped `sticky`/`top-[120px]`/`md:top-[116px]`/`z-20` so it scrolls normally with the page. Confirmed the site-wide header's own `fixed` positioning lives in a completely separate component (`site-header.tsx`) and was unaffected.
- **Navbar visual balance** — `components/storefront/site-header.tsx`. Precise measurement first: the centered column (nav on desktop, logo on mobile) sits at the exact mathematical midpoint of the two equal-width `1fr` flanking columns, on both axes, at every breakpoint checked (also cross-checked against production) — the grid math was never actually broken. The real issue is a visual-weight imbalance: a single 40px logo/menu-button vs. the 3-icon 128px search/bag/account cluster reads as lopsided even though the centering math is correct. First attempt (shifting the centered content itself to compensate) was tested live and looked *worse* — asked before continuing rather than guessing further. Fixed instead by nudging the lighter single-element side (`ml-5` on the desktop logo and the mobile menu button), live-tested at both breakpoints — an eyeballed, live-verified value, not a formula (there isn't one for "how much visual weight is a search icon").

**Byproduct find:** while checking the PLP category sidebar, found 3 orphaned test rows ("RACEFIX-TEST" category/product/variant) left in the dev DB by an earlier session's live-verification script that crashed on a `varchar(30)` overflow before its own cleanup ran — these were live on the real storefront's category nav (dev DB currently also serves production). Deleted directly.
...
- **PDP fixes merged as #131:** size chips were sorting alphabetically (`ORDER BY size ASC` → L, M, S, XL, XS, XXL) instead of garment order — this wasn't just cosmetic, the *default selected variant* used the same order, so every visitor was silently defaulted into size L on every product. New `lib/sizes.ts` (`sortBySize`) fixes both the storefront and admin queries, matching the admin's existing XS→XXL preset order. Also added: gallery arrow navigation + a position counter on the PDP (previously swipe/thumbnail-only, and the mobile rail didn't track position at all — **this whole gallery-arrow/lightbox system was later superseded by a swipe-only mobile redesign, see PR #160's closure note above**), and a safe-area-aware bottom offset for the cart page's toast (the layout opts into `viewportFit: "cover"` for the iPhone notch/home-indicator, but nothing previously reserved `env(safe-area-inset-bottom)`, so a fixed-bottom element could sit partly behind the home indicator on iPhone X+ — **turned out to also protect Android gesture-nav phones and home-button-less iPads for free, since `env(safe-area-inset-*)` is a standard CSS feature, not iOS-only**). `touch-action: pan-y` was also added to the mobile gallery rail at this point (intended to stop a horizontal swipe-carousel nested in a vertically-scrolling page from hijacking a not-perfectly-horizontal swipe) — **found broken and reverted 5 days later (Jul 21, commit `1e7bcbf`): `pan-y` means "only vertical panning is native here," which is backwards from the intent and disabled horizontal swipe entirely on real touch devices. The current gallery ships with no `touch-action` override at all — if you're reading this file for context on the mobile gallery, `touch-action: pan-y` is not live, don't assume it's there.**
...
- **Actual hosting** → Vercel (as planned). **Now on Vercel Pro** (~$20/month) — upgraded post-launch (see "Delivery-status auto-sync" in the status section above), not just for traffic headroom but because the Hobby plan hard-caps cron jobs at once/day with no exceptions, which blocked near-real-time delivery-status polling.
