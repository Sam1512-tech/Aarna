...
### "Google sign-in + OTP codes not working" on prod — diagnosed Jul 15 evening
- **OTP emails never send — Supabase Send-Email hook 404s.** Vercel logs show `[auth] sendEmailOtp error: Unexpected status code returned from hook: 404` — the hook is enabled but still points at the pre-deploy placeholder URL. The deployed route is live and correct (`POST https://aarna-gamma.vercel.app/api/auth/email-hook` → 401 signature-required, as designed). **Fix (dashboard, still TODO):** Supabase → Auth → Hooks → Send Email → point at that URL; the hook secret must match `SUPABASE_AUTH_HOOK_SECRET` already set in Vercel env (regenerate → update env → redeploy if unsure). Note `sendEmailOtp` deliberately returns `ok` even on failure (anti-enumeration), so the UI says "check your inbox" while the send is dying server-side — check Vercel logs, not the UI.
- **Google sign-in reaches Google fine** (provider + client id + Supabase callback all correct — verified live to the accounts.google.com page). The failure is after auth: `aarna-gamma.vercel.app` is not in Supabase's redirect allowlist, so users bounce to the stale Site URL. **Fix (dashboard, still TODO):** Supabase → Auth → URL Configuration → Site URL `https://aarna-gamma.vercel.app` + Redirect URLs add `https://aarna-gamma.vercel.app/**` (keep `http://localhost:3000/**` for dev). No Google-console change needed for the redirect flow.
- **FE bug fixed (#126):** `/login` sent the OTP then handed off to `/login/otp?email=…`, but that page ignored the `email` param and asked for the email again — the just-sent code had nowhere to be typed, and re-sending immediately hit Supabase's 60s OTP rate limit. `LoginOtpView` now takes `initialEmail` and starts at the code-entry step with a 30s resend countdown. Verified live in the browser.
- QA note: test users `aarna-qa-test@example.com` may exist in Supabase Auth from this diagnosis — harmless, delete at leisure.
