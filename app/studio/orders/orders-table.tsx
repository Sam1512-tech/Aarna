...
export function OrdersTable({ items }: { items: AdminOrderListItem[] }) {
  ...
  return (
    <div>
      {error ? <p className="mb-3 text-xs text-burnt-red">{error}</p> : null}
      ...
      {selected.size > 0 ? (
        <div className="sticky bottom-4 z-10 mt-4 flex items-center justify-between gap-3 rounded-full border border-cocoa/15 bg-cream px-5 py-3 shadow-[0_10px_28px_rgba(43,38,35,0.12)]">
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-charcoal/60">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-[11px] font-medium uppercase tracking-[0.16em] text-charcoal/50 transition duration-300 hover:text-charcoal"
            >
              clear
            </button>
            <button
