...
import { markOrderShippedManually } from "@/lib/actions/admin/orders";
import { actionErrorMessage } from "@/lib/action-error";

/**
 * Sits alongside the Create-shipment/Attach-AWB controls for the same "paid,
 * unshipped order" moment — for an order that already went out through
 * something other than Delhivery (a local courier like Porter for an
 * in-city order, or handed over in person) and just needs recording, not
 * booking. Mirrors ShipExchangeManuallyTrigger's exact shape; unlike that
 * one, this doesn't touch stock — a forward order's stock was already
 * decremented at checkout, not at shipment time.
 */
export function ShipOrderManuallyTrigger({ orderId }: { orderId: string }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await markOrderShippedManually(orderId, {
          carrier,
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs leading-5 text-charcoal/60">
        For an order already sent outside Delhivery — a local courier for an
        in-city order, or handed over in person. This only records it; it
        doesn&apos;t book anything.
      </p>
